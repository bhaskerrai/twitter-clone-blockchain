...
contract ProfileImageNfts is ERC721, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns(string memory) {
        require(_exists(tokenId), "URI does not exist on the given ID!");
    ...
    function getAllToken() public view returns(RenderToken[] memory) {
        ...
        for(uint256 i = 0; i <= latestId; i++) {
            if(_exists(i)){
                ...
                res[i] = RenderToken(i, uri, " ");  //solidity, unlike typeScripts, turns interface/structs into functions which passes an object.
            }
        }
